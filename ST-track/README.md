...
## Abstract
Multiple object detection and tracking in aerial images have been widely studied for drone autonomous systems. New challenges have been raised in such remote sensing tasks due to the characteristics of small objects and frequent interleaving. This paper proposes a Spatial-Temporal Feature Aggregation (STFA) module for multiple ship tracking in aerial images. The weights along the temporal dimension could be dynamically learned from inter-frame images while key channel and spatial information within each frame are simultaneously captured. Through the unified optimization of spatial-temporal features, a synthetical spatial-temporal feature representation is ultimately generated. Then, a joint detection and tracking framework based on STFA is developed. Utilizing STFAs as kernel modules, information exchange between parallel multiple resolution feature branches is carried out in the backbone network. And multi-scale spatial-temporal feature can be acquired for detection and re-identification. To illustrate its performance, a specialized dataset collected when drones are on maritime surveillance missions as well as the public dataset VisDrone2019 is utilized in this work. Comprehensive experiments are conducted, an IDF1 score of 92.0% and a MOTA score of 84.6% is achieved on the specialized dataset while the MOTA score has increased by 1.3% on VisDrone2019 dataset. It can be seen that the STFA based tracker is more beneficial to the automation of drones in real-world maritime surveillance.
